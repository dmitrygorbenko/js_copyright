...
function pf($file) {global $PRO_FOLDER; return str_replace($PRO_FOLDER, "", $file); }
$currentYear = date("Y");
$header = str_replace("\r\n", "\n", trim(file_get_contents($PRO_HEADER)));


exec('find '.$PRO_FOLDER.' -name "*.js"', $files);
foreach ($files as $file) {
	if (strpos($file, "jasperserver-war/src/") === false) {
		continue;
	}
	if (checkToSkip($file, $PRO_3PartyItems)) {
		continue;
	}
